...
def run(args):
    ...
    diversity_value = [0.0] * world_size
    n_shapley_round = 0
    # client_list
    factor = [1] * args.world_size
    for i in range(1, args.world_size):
        factor[i] = factor[i - 1] * i
    # cal information between each other
    start_key = 1
    ...
    for i in range(world_size):
        for j in range(world_size):
            ...
            for group_key in range(start_key, end_key + 1):
                ...
                if group_flags[i] == 1 and group_flags[j] == 1:
                    u_with = utility_value[group_key]
                    group_flags[j] = 0
                    group_key = get_utility_key(group_flags)
                    u_without = utility_value[group_key]
                    ij_score += factor[group_size - 1] / float(factor[world_size - 1]) * (u_with - u_without)
            diversity_value[j] = ij_score
            diversity_value[i] = 0.0
        if rank == 0:
