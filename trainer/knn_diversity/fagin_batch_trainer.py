...
class FaginBatchTrainer(object):
    ...
    def find_top_k(self, test_data, test_target, k, group_keys):
        ...
        all_top_k = groups_ind_k[-1]
        # average_dist_top_k = np.average(group_candidate_dist[:, all_top_k], axis=1)
        average_dist_top_k = np.sum(group_candidate_dist[:, all_top_k], axis=1)
