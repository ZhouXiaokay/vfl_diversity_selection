...
from transmission.tenseal_shapley.tenseal_mi_all_reduce_client import AllReduceClient
...
class FaginBatchTrainer(object):
    ...
    def transmit(self, vector, comm_rank):

        summed_vector = self.client.transmit(vector, comm_rank)
    ...
    def find_top_k(self, test_data, test_target, k, group_keys):
        ...
        #         comm_rank = 0
        #         if rank > 1 and group_flags[0] == 1:
        #             comm_rank = 1
        #             group_dist = self.transmit(candidate_local_dist, comm_rank)
        #         elif group_flags[2] == 1 and group_flags[3] == 1:
        #             comm_rank = rank - 2
        #             group_dist = self.transmit(candidate_local_dist, comm_rank)
        #         elif rank == 0 or rank == 1:
        #             group_dist = self.transmit(candidate_local_dist, rank)
        for key in group_keys[:-1]:
            dist.barrier()
            group_flags = utility_key_to_groups(key, self.args.world_size)
            if rank > 0 and group_flags[rank] and group_flags[0] != 1 and group_flags[2] == 1:
                comm_rank = rank - 2

                group_dist = self.transmit(candidate_local_dist, comm_rank)
            elif rank > 0 and group_flags[rank]  and group_flags[0] != 1 and group_flags[2] != 1:
                comm_rank = rank - 1
                group_dist = self.transmit(candidate_local_dist, comm_rank)
            elif group_flags[0] == 1 and group_flags[rank]:
                group_dist = self.transmit(candidate_local_dist, rank)
