...
def get_group_list(lists, group):
    new_list = []
    for rank in range(len(lists)):
        if group[rank]:
            new_list.append(lists[rank])
    return new_list


def master_count_fagin_group(lists, count_arr, cur_top_k, n_k, group):
    group_lists = get_group_list(lists, group)
    n_list = len(group_lists)
